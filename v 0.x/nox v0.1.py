...
with sr.Microphone(device_index=1) as source:               # Включение микровона для записи сообщения на распознование
    ...
    print("Скажи что-нибюудь")                              # Оповещение что можно говорить
...
opts = {
    "alias" :('nox', 'нокс', 'бокс','max','макс','мопс'),
...
speak('Добрый День')
speak('Нокс активирован и готов к работе')

# Остановка прослушивания микрофона что бы произнести свою фразу и не начать говорить самому с собой
stop_listening = r.listen_in_background(m, callback)
while True:
    time.sleep(0.1) #Бесконечный цикл
